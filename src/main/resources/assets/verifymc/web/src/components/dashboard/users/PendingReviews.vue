...
const approveUser = async (user: PendingUser) => {
  ...
  try {
    ...
    if (response.success) {
      ...
      // Try to sync authme, but don't block if it fails
      try {
        await apiService.syncAuthme(locale.value)
      } catch (syncError) {
        console.warn('AuthMe sync failed:', syncError)
      }
      // Always reload the pending users list
...
const confirmReject = async () => {
  ...
  try {
    const response = await apiService.reviewUser({
      userId: rejectDialog.value.user.id,
      action: 'reject',
